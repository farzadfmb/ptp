...
$questionIdAttr = htmlspecialchars((string) ($question['id'] ?? ''), ENT_QUOTES, 'UTF-8');

$defaultAnswers = [];
$defaultCorrectKey = '';
foreach ($questionAnswers as $index => $answer) {
    $uniqueKey = 'ans_' . ((int) ($answer['id'] ?? $index + 1));
    $defaultAnswers[$uniqueKey] = [
        'code' => (string) ($answer['answer_code'] ?? ''),
        'option' => (string) ($answer['option_text'] ?? ''),
        'score_numeric' => isset($answer['numeric_score']) ? (string) $answer['numeric_score'] : '',
        'score_character' => (string) ($answer['character_score'] ?? ''),
        'display_order' => isset($answer['display_order']) ? (string) $answer['display_order'] : (string) ($index + 1),
        'is_correct' => (int) ($answer['is_correct'] ?? 0),
    ];
    if ((int) ($answer['is_correct'] ?? 0) === 1) {
        $defaultCorrectKey = $uniqueKey;
    }
}

if ($defaultCorrectKey === '' && !empty($defaultAnswers)) {
    $defaultCorrectKey = (string) array_key_first($defaultAnswers);
}
...
$oldAnswers = old('answers', $defaultAnswers);
$oldCorrectAnswer = (string) old('correct_answer', $defaultCorrectKey);
...
if (is_array($oldAnswers)) {
    foreach ($oldAnswers as $key => $answer) {
        $preparedAnswers[] = [
            ...
            'is_correct' => ((string) $key === $oldCorrectAnswer) ? 1 : ((int) ($answer['is_correct'] ?? 0)),
        ];
    }
}
...
if (empty($preparedAnswers)) {
    $preparedAnswers = [];
    $index = 1;
    foreach ($defaultAnswers as $key => $answer) {
        $preparedAnswers[] = [
            'key' => (string) $key,
            'code' => (string) ($answer['code'] ?? ''),
            'option' => (string) ($answer['option'] ?? ''),
            'score_numeric' => (string) ($answer['score_numeric'] ?? ''),
            'score_character' => (string) ($answer['score_character'] ?? ''),
            'display_order' => (string) ($answer['display_order'] ?? (string) $index),
            'is_correct' => (int) ($answer['is_correct'] ?? 0),
        ];
        $index++;
    }
...
if (empty($preparedAnswers)) {
    ...
    $oldCorrectAnswer = $preparedAnswers[0]['key'];
}
...
$initialCorrectAnswerJson = json_encode($oldCorrectAnswer !== '' ? $oldCorrectAnswer : ($preparedAnswers[0]['key'] ?? ''), JSON_UNESCAPED_UNICODE | JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_AMP | JSON_HEX_QUOT);
                ...
                <div class="card question-form-card shadow-sm h-100">
                    <div class="card-body p-24">
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-16 mb-24">
                            <div>
                                <div class="question-meta mb-8">
                                    <h2 class="mb-0 text-gray-900">ویرایش سوال برای <?= $toolName; ?></h2>
                                    <?php if ($toolCode !== ''): ?>
                                        <span><ion-icon name="barcode-outline"></ion-icon>کد ابزار: <?= $toolCode; ?></span>
                                    <?php endif; ?>
                                </div>
                                <p class="text-gray-500 mb-0">جزئیات سوال را بروزرسانی کنید و در صورت نیاز گزینه‌های پاسخ را اصلاح نمایید.</p>
                            </div>
                        ...
                        <form action="<?= UtilityHelper::baseUrl('organizations/evaluation-tools/questions/update'); ?>" method="post" enctype="multipart/form-data" class="question-form">
                            <?= csrf_field(); ?>
                            <input type="hidden" name="tool_id" value="<?= $toolIdAttr; ?>">
                            <input type="hidden" name="id" value="<?= $questionIdAttr; ?>">

                            <div class="row g-16">
                                <div class="col-md-4">
                                    <label class="form-label">کد سوال <span class="text-danger">*</span></label>
                                    <input type="text" name="code" class="form-control" placeholder="مثال: Q-01" value="<?= htmlspecialchars(old('code', $question['code'] ?? ''), ENT_QUOTES, 'UTF-8'); ?>" required>
                                ...
                                <div class="col-md-4">
                                    ...
                                    <input type="number" name="display_order" class="form-control" min="0" placeholder="مثال: 1" value="<?= htmlspecialchars(old('display_order', isset($question['display_order']) ? (string) $question['display_order'] : ''), ENT_QUOTES, 'UTF-8'); ?>">
                                    ...
                                    <?php else: ?>
                                        <div class="form-text">اگر مقدار را خالی بگذارید، بر اساس ترتیب ثبت مرتب می‌شود.</div>
                                    <?php endif; ?>
                                </div>
                                <div class="col-md-4">
                                    <label class="form-label">فقط توضیح می‌باشد؟</label>
                                    <?php $oldDescriptionOnly = (string) old('is_description_only', (string) ($question['is_description_only'] ?? '0')); ?>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea name="question" class="form-control" placeholder="متن کامل سوال را وارد کنید" required><?= htmlspecialchars(old('question', $question['question_text'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <textarea name="description" class="form-control" placeholder="توضیح تکمیلی برای سوال (اختیاری)"><?= htmlspecialchars(old('description', $question['description'] ?? ''), ENT_QUOTES, 'UTF-8'); ?></textarea>
                                </div>
                                <div class="col-md-6">
                                ...
                                <?php if (!empty($question['image_path'])): ?>
                                    <div class="col-md-6">
                                        ...
                                        <div class="d-flex align-items-center gap-12">
                                            <img src="<?= UtilityHelper::baseUrl('public/' . ltrim($question['image_path'], '/')); ?>" alt="تصویر سوال" class="question-image-preview">
                                            <div class="form-check mt-2">
                                                <input class="form-check-input" type="checkbox" name="remove_image" value="1" id="remove-image-checkbox">
                                                <label class="form-check-label" for="remove-image-checkbox">حذف تصویر فعلی</label>
                                            </div>
                                        </div>
                                    </div>
                                <?php endif; ?>
