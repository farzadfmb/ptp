...
$title = $title ?? 'ویرایش تیپ شخصیتی DISC';
...
$discType = $discTypeRecord ?? ($discType ?? []);
if (!empty($oldInput)) {
    $discType = array_merge($discType, array_filter($oldInput, static function ($value) {
        return $value !== null;
    }));
}
$oldInput = $oldInput ?? [];

$currentScope = old('scope', (string) ($discType['scope'] ?? array_key_first($scopeMeta) ?? 'primary'));
$cancelUrl = UtilityHelper::baseUrl('organizations/tools/disc-settings?scope=' . urlencode($currentScope));
                    ...
                    <div class="card-body p-24">
                        <div class="d-flex flex-wrap justify-content-between align-items-center gap-16 mb-24">
                            <div class="text-start flex-grow-1">
                                <h2 class="mb-6 text-gray-900">ویرایش تیپ شخصیتی DISC</h2>
                                <p class="text-gray-500 mb-0">تغییرات مورد نظر را اعمال کنید و سپس ذخیره نمایید.</p>
                            </div>
                        ...
                        <?php
                            $formAction = UtilityHelper::baseUrl('organizations/tools/disc-settings/update');
